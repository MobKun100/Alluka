...
function addXP(userId, guildId) {
  ...
  if (userData.xp >= requiredXP) {
    ...
    // Ödül sistemi: her seviye için belirli bir miktar ödül
    const rewards = {
      1: 100,
      2: 200,
      3: 300,
  ...
  if (command === "ship") {
    ...
    if (!mentionedUser) {
      return message.reply(
        "Kimi ile shipleyelim? Birini etiketle! Örnek: `!ship @kullanıcı`",
      );
    }
    if (mentionedUser.id === message.author.id) {
      return message.reply("Kendini kendine shipliyemezsin! 😅");
    }
    // Rastgele yüzde hesapla (0-100 arası)
    ...
    // Kullanıcıya sonucu gönder
    return message.channel.send(
      `${message.author.username} ve ${mentionedUser.username} arasındaki uyum oranı: **${shipPercentage}%** - ${shipStatus}`,
    );
  }
});
...
client.on("messageCreate", async (message) => {
  ...
  const sendHelpEmbed = () => {
    const yardımEmbed = new EmbedBuilder()
      ...
      .setDescription(
        "**Tüm komutlar ve açıklamaları kategorilere göre ayrılmıştır!**",
      )
  ...
  if (command === "bakiye" || command === "balance" || command === "bal") {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `${message.author.tag} adlı kullanıcının bakiyesi: **${formatCoins(coins)}**`,
      )
      .setThumbnail(message.author.displayAvatarURL({ dynamic: true }))
      .setTimestamp();
  ...
  if (command === "daily") {
    ...
    if (cooldownTime > Date.now()) {
      ...
      return message.reply(
        `⏰ Günlük ödülünü zaten aldın! **${timeLeft}** sonra tekrar dene.`,
      );
    }
  ...
  if (command === "work") {
    ...
    if (cooldownTime > Date.now()) {
      const timeLeft = formatTime(cooldownTime - Date.now());
      return message.reply(
        `⏰ Henüz çalışamazsın! **${timeLeft}** sonra tekrar dene.`,
      );
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `${selectedWork} ve **${formatCoins(earnedAmount)}** kazandın!`,
      )
  ...
  if (command === "slot") {
    ...
    if (userCoins < amount) {
      return message.reply(
        `Yetersiz bakiye! Bakiyen: **${formatCoins(userCoins)}**`,
      );
    }
  ...
  if (command === "coinflip" || command === "cf") {
    ...
    if (!amount || amount < 1) {
      return message.reply(
        "Geçerli bir miktar gir! Örnek: `!coinflip 100 yazı`",
      );
    }
  ...
  if (command === "dice") {
    ...
    if (!guess || guess < 1 || guess > 6) {
      return message.reply(
        "1-6 arası bir sayı tahmin et! Örnek: `!dice 100 3`",
      );
    }
  ...
  if (command === "hunt") {
    ...
    if (success) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `${caughtAnimal.name} yakaladın ve **${formatCoins(caughtAnimal.value)}** kazandın!`,
        )
        .setTimestamp();
  ...
  if (command === "fish") {
    ...
    if (success) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `${caughtFish.name} tuttun ve **${formatCoins(caughtFish.value)}** kazandın!`,
        )
        .setTimestamp();
