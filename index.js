...
function checkAndResetStats(stats) {
  ...
  if (stats.weekly.periodStart < getWeekStart(now))
    stats.weekly = { messages: 0, voiceMinutes: 0, periodStart: getWeekStart(now) };
  if (stats.monthly.periodStart < getMonthStart(now))
    stats.monthly = { messages: 0, voiceMinutes: 0, periodStart: getMonthStart(now) };
}
...
// ── Mod rol yardımcısı ────────────────────────────────────────────────────────
// command: "ban" | "kick" | "mute" | "warn" | "temizle"
// fallbackPerm: PermissionsBitField.Flags.*
...
// ── Kullanıcı çözümleyici (mention veya ID) ───────────────────────────────────
async function resolveUser(message, args) {
  const mention = message.mentions.members.first();
  if (mention) return mention;
  if (args[0]) {
    try { return await message.guild.members.fetch(args[0]); } catch {}
  }
  return null;
}
...
client.on("voiceStateUpdate", (oldState, newState) => {
  ...
  if (joined) {
    ...
    sendLog(guild, "ses", new EmbedBuilder()
      .setColor("Green").setTitle("🔊 Ses Kanalına Katıldı")
      .addFields(
        { name: "Kullanıcı", value: `${member.user.tag} (<@${userId}>)`, inline: true },
        { name: "Kanal", value: ch ? `${ch.name}` : "Bilinmiyor", inline: true },
    ...
    sendLog(guild, "ses", new EmbedBuilder()
      .setColor("Red").setTitle("🔇 Ses Kanalından Çıktı")
      .addFields(
        { name: "Kullanıcı", value: `${member.user.tag} (<@${userId}>)`, inline: true },
        { name: "Kanal", value: ch ? `${ch.name}` : "Bilinmiyor", inline: true },
      ).setFooter({ text: `ID: ${userId}` }).setTimestamp());
    ...
    sendLog(guild, "ses", new EmbedBuilder()
      .setColor("Blue").setTitle("🔀 Ses Kanalı Değiştirdi")
      .addFields(
        { name: "Kullanıcı", value: `${member.user.tag} (<@${userId}>)`, inline: true },
        { name: "Eski Kanal", value: oldCh ? oldCh.name : "?", inline: true },
        { name: "Yeni Kanal", value: newCh ? newCh.name : "?", inline: true },
      ).setFooter({ text: `ID: ${userId}` }).setTimestamp());
  }
});

// ── Davet log — üye katıldı ───────────────────────────────────────────────────
client.on("guildMemberAdd", async (member) => {
  ...
  const inviteInfo = usedInvite
    ...
    : "Bilinmiyor (vandalizm veya bot tarafından eklendi)";

  sendLog(guild, "davet", new EmbedBuilder()
    .setColor("Green").setTitle("📥 Üye Katıldı")
    .setThumbnail(member.user.displayAvatarURL({ dynamic: true }))
    .addFields(
...
// ── Davet log — üye ayrıldı ───────────────────────────────────────────────────
client.on("guildMemberRemove", (member) => {
  sendLog(member.guild, "davet", new EmbedBuilder()
    .setColor("Red").setTitle("📤 Üye Ayrıldı")
    .setThumbnail(member.user.displayAvatarURL({ dynamic: true }))
    .addFields(
      { name: "Kullanıcı", value: `${member.user.tag} (<@${member.id}>)`, inline: true },
...
client.on("messageUpdate", (oldMsg, newMsg) => {
  ...
  sendLog(oldMsg.guild, "mesaj", new EmbedBuilder()
    .setColor("Yellow").setTitle("✏️ Mesaj Düzenlendi")
    .addFields(
      { name: "Kullanıcı", value: `${oldMsg.author?.tag || "Bilinmiyor"} (<@${oldMsg.author?.id}>)`, inline: true },
...
client.on("messageDelete", (msg) => {
  ...
  sendLog(msg.guild, "mesaj", new EmbedBuilder()
    .setColor("Red").setTitle("🗑️ Mesaj Silindi")
    .addFields(
      ...
      { name: "Silinen Mesaj", value: (msg.content || "*içerik yok (resim/dosya)*").slice(0, 1024), inline: false },
...
client.on("messageCreate", async (message) => {
  ...
  if (command === "yardım" || command === "yardim" || command === "help") {
    const embed = new EmbedBuilder()
      .setColor("#FF6B6B").setTitle("🎮 Bot Komut Rehberi")
      ...
      .addFields(
        { name: "🛡️ **MODERASYON**", value: [
          ...
          "`a!mute @üye/ID <süre>` • Susturur (10m, 1h, 1d)",
          "`a!unmute @üye/ID` • Susturmayı kaldırır",
          "`a!warn @üye/ID <sebep>` • Warn atar",
          "`a!warnings [@üye/ID]` • Warn geçmişini listeler",
          "`a!unwarn @üye/ID <numara|all>` • Warn siler",
        ].join("\n"), inline: false },
        { name: "⚙️ **MOD ROL AYARLARI** *(Admin)*", value: [
          "`a!warnrol @rol` • Warn atabilecek rolü ayarlar",
          "`a!banrol @rol` • Ban atabilecek rolü ayarlar",
          "`a!kickrol @rol` • Kick atabilecek rolü ayarlar",
          "`a!muterol @rol` • Mute yapabilecek rolü ayarlar",
          "`a!temizlerol @rol` • Temizle yapabilecek rolü ayarlar",
          "`a!modroller` • Mevcut mod rollerini gösterir",
        ].join("\n"), inline: false },
        { name: "📋 **LOG SİSTEMİ** *(Admin)*", value: [
          "`a!logayarla ses/davet/mesaj #kanal` • Log kanalını ayarlar & açar",
          "`a!logkapat ses/davet/mesaj` • O log türünü kapatır",
          "`a!logac ses/davet/mesaj` • O log türünü açar",
          "`a!logdurum` • Log ayarlarını gösterir",
        ].join("\n"), inline: false },
        { name: "👤 **PROFİL & MAĞAZA**", value: [
          ...
          "`a!profilayarla bio <metin>` • Bio ayarlar",
  ...
  if (command === "level" || command === "xp") {
    ...
    return message.channel.send({ embeds: [
      new EmbedBuilder().setColor(getProfile(target.id, message.guild.id).color || "Purple")
        .setTitle("📊 Level Bilgisi").setDescription(`${target.user.tag} kullanıcısının seviyesi`)
        .addFields(
  ...
  if (command === "leaderboard" || command === "lb" || command === "sıralama") {
    ...
    return message.channel.send({ embeds: [
      new EmbedBuilder().setColor("Gold").setTitle("🏆 Liderlik Tablosu")
        .addFields(
          { name: "📊 Level Sıralaması", value: lvlData.slice(0,10).map((u,i)=>`${medal(i)} **${u.username}** — Level ${u.level} (${u.xp} XP)`).join("\n") || "Henüz veri yok", inline: false },
          { name: "💰 Hunter Bucks Sıralaması", value: coinData.slice(0,10).map((u,i)=>`${medal(i)} **${u.username}** — ${formatCoins(u.coins)}`).join("\n") || "Henüz veri yok", inline: false },
  ...
  if (command === "profilayarla" || command === "profil-ayarla") {
    ...
    if (!sub) return message.reply([
      "Nasıl kullanılır?",
      "`a!profilayarla renk <renk_id>`",
      "`a!profilayarla rozet <rozet_id>`",
    ...
    return message.channel.send({ embeds: [
      new EmbedBuilder().setColor(prof.color)
        ...
        .addFields(
          ...
          { name: "🏅 Aktif Rozet",  value: prof.activeBadge || "Yok", inline: true },
  ...
  if (command === "istatistik" || command === "stats") {
    ...
    return message.channel.send({ embeds: [
      new EmbedBuilder().setColor(getProfile(target.id, message.guild.id).color || "#5865F2")
        ...
        .setThumbnail(target.user.displayAvatarURL({ dynamic: true }))
        .addFields(
          { name: `📅 Bu Hafta (${wStart}'den beri)`, value: [`💬 Mesaj: **${stats.weekly.messages.toLocaleString()}**`, `🔊 Ses Süresi: **${formatDuration(stats.weekly.voiceMinutes)}**`].join("\n"), inline: false },
          { name: `🗓️ Bu Ay (${mStart}'den beri)`,    value: [`💬 Mesaj: **${stats.monthly.messages.toLocaleString()}**`, `🔊 Ses Süresi: **${formatDuration(stats.monthly.voiceMinutes)}**`].join("\n"), inline: false },
  ...
  if (command === "dice") {
    ...
    return message.channel.send({ embeds: [new EmbedBuilder().setColor(won?"Green":"Red").setTitle("🎲 Zar Oyunu").setDescription(`Zar: **${result}** | Tahminin: **${guess}**\n\n${won?`🎉 Kazandın! **${formatCoins(amount*5)}**`:`😢 Kaybettin! **${formatCoins(amount)}** kaybettin.`}`).setTimestamp()] });
  }

  if (command === "hunt") {
    const animals = [{name:"🐰 Tavşan",value:50},{name:"🦌 Geyik",value:100},{name:"🐗 Domuz",value:150},{name:"🐺 Kurt",value:200},{name:"🐻 Ayı",value:300},{name:"🦁 Aslan",value:500}];
  ...
  if (command === "fish") {
    const fishes = [{name:"🐟 Küçük Balık",value:30},{name:"🐠 Renkli Balık",value:60},{name:"🦈 Köpekbalığı",value:200},{name:"🐋 Balina",value:400},{name:"🦑 Ahtapot",value:100},{name:"🦀 Yengeç",value:80}];
  ...
  if (command === "modroller") {
    ...
    const lines = keys.map(k => {
      const roleId = guildRoles[k];
      const roleStr = roleId ? `<@&${roleId}>` : "*Ayarlanmadı (Discord yetkisi kullanılır)*";
      return `**${k}** → ${roleStr}`;
    });
    return message.channel.send({ embeds: [
      new EmbedBuilder().setColor("#5865F2").setTitle("⚙️ Mod Rolleri")
        .setDescription(lines.join("\n"))
        .setFooter({ text: "Değiştirmek için: a!warnrol / a!banrol / a!kickrol / a!muterol / a!temizlerol" }).setTimestamp(),
    ]});
  }
  ...
  if (command === "logayarla") {
    ...
    if (!logTypes[type]) return message.reply("Geçerli türler: `ses`, `davet`, `mesaj`\nÖrnek: `a!logayarla ses #log-kanalı`");
    const ch = message.mentions.channels.first();
    if (!ch) return message.reply(`Bir kanal etiketle! Örnek: \`a!logayarla ${type} #log-kanalı\``);
    ...
    saveData(logSettingsFile, logSettingsMap);
    return message.channel.send({ embeds: [
      new EmbedBuilder().setColor("Green").setTitle("✅ Log Ayarlandı")
        .setDescription(`${logTypes[type]} artık ${ch} kanalına gönderilecek ve **açık**.`).setTimestamp(),
    ]});
  }
  ...
  if (command === "logkapat") {
    ...
    saveData(logSettingsFile, logSettingsMap);
    return message.channel.send({ embeds: [
      new EmbedBuilder().setColor("Red").setTitle("🔕 Log Kapatıldı")
        .setDescription(`${logTypes[type]} artık **kapalı**.`).setTimestamp(),
    ]});
  }
  ...
  if (command === "logac" || command === "logaç") {
    ...
    saveData(logSettingsFile, logSettingsMap);
    return message.channel.send({ embeds: [
      new EmbedBuilder().setColor("Green").setTitle("🔔 Log Açıldı")
        .setDescription(`${logTypes[type]} artık **açık**.`).setTimestamp(),
    ]});
  }
  ...
  if (command === "logdurum") {
    ...
    const lines = Object.entries(logTypes).map(([key, label]) => {
      ...
      const status = cfg.enabled ? "✅ Açık" : "❌ Kapalı";
      const channel = cfg.channelId ? `<#${cfg.channelId}>` : "*Kanal ayarlanmadı*";
      return `${label}\n↳ ${status} | Kanal: ${channel}`;
    });
    return message.channel.send({ embeds: [
      new EmbedBuilder().setColor("#5865F2").setTitle("📋 Log Sistemi Durumu")
        .setDescription(lines.join("\n\n"))
  ...
  // ── WARN SİSTEMİ ────────────────────────────────────────────────────────────
  if (command === "warn") {
    if (!hasModPerm(message.member, "warn", PermissionsBitField.Flags.ModerateMembers))
    ...
    const reason = (message.mentions.users.size > 0 ? args.slice(1) : args.slice(1)).join(" ");
  ...
  if (command === "warnings" || command === "warnlist") {
    const target = message.mentions.members.first() || await (async () => {
  ...
  if (command === "unwarn") {
    ...
    const warns = warningsMap.get(warnKey) || [];
    if (warns.length === 0) return message.reply(`${target.user.tag} adlı kullanıcının hiç warni yok.`);
    const sub = (message.mentions.users.size > 0 ? args[1] : args[1])?.toLowerCase();
    if (sub === "all" || sub === "hepsi") {
      warningsMap.set(warnKey, []);
      saveData(warningsFile, warningsMap);
      return message.channel.send({ embeds: [
        new EmbedBuilder().setColor("Green").setTitle("✅ Tüm Warnlar Silindi")
          .setDescription(`${target.user.tag} adlı kullanıcının **${warns.length}** warni silindi.`).setTimestamp(),
      ]});
    }
  ...
  // ── MODERASYON ──────────────────────────────────────────────────────────────
  if (command === "ban") {
    if (!hasModPerm(message.member, "ban", PermissionsBitField.Flags.BanMembers))
      return message.reply("🚫 Ban yetkin yok!");
    const target = await resolveUser(message, args);
    if (!target) return message.reply("Kullanıcı bulunamadı! Mention veya ID gir.");
    if (!target.bannable) return message.reply("🚫 Bu kullanıcıyı banlayamam!");
    const reason = (message.mentions.users.size > 0 ? args.slice(1) : args.slice(1)).join(" ") || "Sebep belirtilmedi";
  ...
  if (command === "kick") {
    if (!hasModPerm(message.member, "kick", PermissionsBitField.Flags.KickMembers))
  ...
  if (command === "temizle" || command === "clear" || command === "purge") {
    if (!hasModPerm(message.member, "temizle", PermissionsBitField.Flags.ManageMessages))
  ...
  if (command === "mute") {
    if (!hasModPerm(message.member, "mute", PermissionsBitField.Flags.ModerateMembers))
    ...
    const reason = (message.mentions.users.size > 0 ? args.slice(2) : args.slice(2)).join(" ") || "Sebep belirtilmedi";
